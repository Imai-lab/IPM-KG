...
class Main(object):
	...
	def add_model(self):
		model = KG_PREDICT(self.edge_index, self.edge_type, self.chequer_perm, params=self.p)
	...
	def testpredict(self, split=''):
		...
		with torch.no_grad():
			...
			for step, batch in enumerate(train_iter):
				...
				print(len(torch.argsort(torch.argsort(pred, dim=1, descending=True), dim=1, descending=False)))
				print(len(torch.argsort(torch.argsort(pred, dim=1, descending=True), dim=1, descending=False)[0]))
