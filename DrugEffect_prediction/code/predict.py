...
from model.GPKG_conv import KGConv
from model.GPKG_conv_basis import KGConvBasis
...
class GPKG_EMBEDD(BaseModel):
	def __init__(self, edge_index, edge_type, num_rel, params=None):
		...
		if self.p.num_bases > 0:
			self.conv1 = KGConvBasis(self.p.init_dim, self.p.gcn_dim, num_rel, self.p.num_bases, act=self.act, params=self.p)
			self.conv2 = KGConv(self.p.gcn_dim,    self.p.embed_dim,    num_rel, act=self.act, params=self.p) if self.p.gcn_layer == 2 else None
		else:
			self.conv1 = KGConv(self.p.init_dim, self.p.gcn_dim,      num_rel, act=self.act, params=self.p)
			self.conv2 = KGConv(self.p.gcn_dim,    self.p.embed_dim,    num_rel, act=self.act, params=self.p) if self.p.gcn_layer == 2 else None
